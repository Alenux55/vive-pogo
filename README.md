...
There are **14 unique jobs**. Fabrication runs ERC, DRC, four native exports and
a formatter (7 jobs). Assembly runs ERC, DRC, six native exports and a formatter
(9 jobs). One run produces **23 fabrication files and 8 assembly files**.

```text
Releases/
  Fabrication/
    Checks/                         ERC, DRC, formatter log
...
The project has no named variants. Files, folders, BOM metadata and base assembly
sheet labels use **No Variant**, labelling KiCad's empty/base selection rather
than creating an electrical variant. Fabrication remains variant-neutral.
...
`Assembly-Top.kicad_wks` and `Assembly-Bottom.kicad_wks` provide the view labels
and page numbers 1/2 and 2/2. Joining preserves vector content, scale and
...
title, merged metadata cells, fonts, grey fills, widths and six-column layout.
Body rows wrap text. Quantities/line numbers are numeric; identifiers/revisions
...
Validated on 2026-09-18: 7/7 fabrication and 9/9 assembly jobs passed. Both merged
...
and rendered against the template. Six formatter tests cover merge order,
metadata/types, literal text, missing exports, schema mismatch and context/path
guards. Actual local Prism handler/resolver/Assets code is exercised with real
